...
import { returnLetterBagInputConfiguration } from '../../../Utility/InputConfiguration';
...
class CreateBag extends Component {
    ...
    componentDidMount = () =>{
        this.setState({ bagForm: returnLetterBagInputConfiguration() });
    }
    ...
    createBag = (event) => {
		...
		const bagToCreate = {
			...
			type: 1,
			letterCount: parseInt(this.state.bagForm.letterCount.value),
			weight: parseFloat(this.state.bagForm.weight.value),
            price: parseFloat(this.state.bagForm.price.value)
		}
        ...
        return (
            <div className="well" key={'div1'}>
                ...
                <SuccessModal show={this.props.showSuccessModal} 
					   ...
					   modalBodyText={'Bag of letters created successfully'}
					   okButtonText={'OK'} 
